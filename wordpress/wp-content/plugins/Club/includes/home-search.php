<?php function home_search_shortcode(){
  ...
  $sports_facility = get_post_meta($post->ID,'sports_facility', true);
      ...
      <!-- <input type="hidden" name="post_type" value="clubjobs" /> -->
        <div class="row custom-row">
                    ...
                    <select id="stadtteil" name="sports_facility" class="no-combobox form-control">
                              ...
                              <?php 
                                ...
                                while ($query->have_posts()) {
                                    $query->the_post();
                                    $post_id = get_the_ID(); ?>
                                    <option value="<?php echo $post_id; ?>" <?php selected( $_POST['sports_facility'], $post_id ); ?>><?php the_title(); ?></option>
                                <?php }
          ...
          <div class="custom-col custom-col-md-6 custom-col-xl-3">
            <div class="custom-form-group">
              <label for="gym_sport_facility"><?php _e('Keyword'); ?></label>
                <input type="text" name="search_keyword" id="s" <?php if(is_search()) { ?>value="<?php the_search_query(); ?>" <?php } else { ?>value="" onfocus="if(this.value==this.defaultValue)this.value='';" onblur="if(this.value=='')this.value=this.defaultValue;"<?php } ?> />
            </div>
          </div>
         ...
         <div class="search-submit contact-footer">
                  <input type="submit" name="search-theme" id="search-theme" value="Submit" />
                  <input type="reset" name="search-theme-reset" id="search_theme_reset" class="grey-btn" value="Reset" />
          </div>
</form>  
</div>
<?php
...
/*  if(isset($_POST['subbtn'])){
  //echo "string";
  $sport = $_POST['facility'];
  $target_group = $_POST['jobs-type'];
  $township = $_POST['township'];
  // echo $sport;
  // echo $target_group;
  // echo $township;
  $district = $wpdb->get_results("select * from {$wpdb->base_prefix}postmeta where meta_value LIKE '%$sport%' OR meta_value LIKE '%$target_group%' OR meta_value LIKE '%$township%'");
  get_search_form( $district );
  echo "<pre>";
  print_r($district);
  if($district){
    foreach($district as $data){
      echo $data->post_id;
      $id = $data->post_id;
      $job = $wpdb->get_results("select * from {$wpdb->base_prefix}posts where ID ='$id'");
      print_r($job);
      foreach($job as $data){
        echo "hdd";
        echo $data->job_title;
  ...
  // echo $district['2']['meta_key'];
  }*/
}

add_shortcode('JobSearch','home_search_shortcode'); ?>
