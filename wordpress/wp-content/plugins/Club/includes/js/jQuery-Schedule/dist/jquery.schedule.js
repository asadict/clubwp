...
;(function ($, window, document, undefined) {
  ...
  function Plugin(element, options) {
    ...
    this.periodOptions = {
      ...
      paddingTop: this.settings.periodpaddingTop
    };
    this.init();
  }
  ...
  $.extend(Plugin.prototype, {
    ...
    init: function () {
      ...
      // duration validation
      if ($.inArray(this.settings.periodDuration, [5]) === -1) {
    ...
    generate: function () {
      ...
      if (this.settings.disableData.length > 0) {
        var $this = this;
        $.each(this.settings.disableData, function (index, disableData) {
          $.each(disableData.periods, function (index, period) {

            var parent = $('.jqs-day', $this.element).eq(disableData.day);
            var options = {};
            var height, position;
            if ($.isArray(period)) {
              position = $this.positionFormat(period[0]);
     ...
     * @param options
     */
    add: function (parent, position, height, options,disableClass="") {
      ...
      var periodRemove = '';
      var periodDuplicate = '';
      if (this.settings.mode === 'edit' && disableClass === '') {
      ...
      var period = $('<div class="jqs-period '+disableClass+'">' +
   ...
   /*     period.draggable({
        ...
        if (this.settings.periodOptions && disableClass === '') {
      ...
      if (!this.settings.onRemovePeriod.call(this, period, this.element)) {
        period.remove();
         ...
         isFlag = false;
                ...
                if(isFlag == true){
                  $('#result').val(JSON.stringify(tempData));
                }else{
                  // $('#time').val("");
                  // $('#timeEnd').val("");
                  $('#result').val("");
    ...
    openOptions: function (event, period) {
     ...
     // var duplicate = '<div class="jqs-options-duplicate">' + this.settings.periodDuplicateButton + '</div>';
      ...
      $('<div class="jqs-options"> <div class="day-select">\n' +
          '<div class="custom-col custom-col-md-12 custom-hide-div gym_day_and_time">\n' +
    ...
    periodData: function (period) {
      ...
      return {
        ...
        paddingTop: $('.jqs-period-container', period).css('padding-top'),
      };
    },
decimalAdjust: function(type, value, exp) {
  ...
  return +(value[0] + 'e' + (value[1] ? (+value[1] + exp) : exp));
    ...
    periodFormat: function (position) {
     ...
     // var mn = Math.ceil((position / this.periodInterval - hour) * 60);
          //  var mn = (position / this.periodInterval - hour) * 60;
     var mn =     Math.trunc(this.decimalAdjust('ceil', ((position / this.periodInterval - hour) * 60), -1));
    ...
    importDisable: function (args) {
      var $this = this;
      var dataImport = args[1];
      var ret = [];

      $.each(dataImport, function (index, disableData) {

        $.each(disableData.periods, function (index, period) {
          $('.jqs-day', $this.element).children('.jqs-period').addClass('disable');
         // console.log($('.jqs-day').children('.jqs-period').);
          var parent = $('.jqs-day', $this.element).eq(disableData.day);
          ...
          var status = true;
          if (!$this.add(parent, position, height - position, options,'disable')) {
            status = false;
          }
